    ...
    return (
      <div>
        ...
        <div className="container">
            <div className="row">
                <div class="col-md-9 mt-5 tab-content" id="myTabContent">
                    <NavPill />
                    {
                      listArticles.map((item, index)=>(
                        <div className="article-privew" key={index}>
                        <PostingOwner  userName={item.author.username} date={item.createdAt} 
                          // parentCallback={callBackFunction}
                        />
                        <PostingContent title={item.title} description={item.description} 
                  ...
                  <ul className="Pagination">
                    {
                      Array(articlesCount/10).fill(1).map((item, index)=>(
